...
class SilverIngot(itemService: ItemService, type: CustomItemType) : CustomItemBlueprint(itemService, type), ISmeltable,
     ...
     */
    override fun getIngredient(): RecipeChoice { return ExactChoice(generate(CustomItemType.RAW_SILVER)) }
    ...
    override val compressor: CompressionStep? get() = when (customItemType) {
        CustomItemType.SILVER_INGOT ->
            CompressionStep(itemService.getBlueprint(CustomItemType.SILVER_BLOCK) as ICompressible, 9, 1)
        else -> null
    }
}
