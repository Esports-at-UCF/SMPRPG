...
private const val WORTH_PER_ITEM = 5

class RawTin(itemService: ItemService, type: CustomItemType) :
    ...
    override val compressor: CompressionStep? get() = when (customItemType) {
        CustomItemType.RAW_TIN ->
            CompressionStep(itemService.getBlueprint(CustomItemType.RAW_TIN_BLOCK) as ICompressible, 9, 1)
        else -> null
    }
