...
class AetheriumSpear(itemService: ItemService, type: CustomItemType) : AetheriumAttributeItem(itemService, type), ICraftable, IBreakableEquipment, IModelOverridden {
    ...
    companion object {
        val KINETIC_COMP : KineticWeapon = KineticWeapon.kineticWeapon()
            .dismountConditions(KineticWeapon.condition(100, 14.0f, 0.0f))
            .forwardMovement(0.38f)
            .hitSound(SoundEventKeys.ITEM_SPEAR_HIT)
            .sound(SoundEventKeys.ITEM_SPEAR_USE)
            .knockbackConditions(KineticWeapon.condition(200, 5.1f, 0.0f))
            .delayTicks(15)
            .damageMultiplier(0.85f)
            .damageConditions(KineticWeapon.condition(300, 0.0f, 4.6f))
            .build()
    }
}
