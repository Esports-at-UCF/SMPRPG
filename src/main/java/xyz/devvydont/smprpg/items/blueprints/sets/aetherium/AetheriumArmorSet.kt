...
abstract class AetheriumArmorSet(itemService: ItemService, type: CustomItemType) :
    ...
    val toolStats : ToolStats get() = ToolStats.AETHERIUM
    ...
    open fun getCraftingMaterial(): ItemStack = itemService.getCustomItem(CustomItemType.AETHERIUM_INGOT)

    open fun unlockedBy(): MutableCollection<ItemStack?>? { return mutableListOf(
        itemService.getCustomItem(
            CustomItemType.AETHERIUM_INGOT
        )
    )
    }
    ...
    companion object {
        private val key = Key.key(plugin, "aetherium")
    }
}
