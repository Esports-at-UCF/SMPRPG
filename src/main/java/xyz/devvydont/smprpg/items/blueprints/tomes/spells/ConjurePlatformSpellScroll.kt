...
import xyz.devvydont.smprpg.ability.handlers.ConjurePlatformAbilityHandler
import xyz.devvydont.smprpg.items.CustomItemType
import xyz.devvydont.smprpg.items.interfaces.IAbilityCaster.AbilityEntry
import xyz.devvydont.smprpg.services.ItemService

class ConjurePlatformSpellScroll(itemService: ItemService, type: CustomItemType) : SpellBlueprint(itemService, type) {
    override fun getAbilities(item: ItemStack): Collection<AbilityEntry> {
        return mutableListOf(
            AbilityEntry(
                Ability.CONJURE_PLATFORM,
    ...
    override fun getCooldown(item: ItemStack): Long {
        return ConjurePlatformAbilityHandler.COOLDOWN
    }
}