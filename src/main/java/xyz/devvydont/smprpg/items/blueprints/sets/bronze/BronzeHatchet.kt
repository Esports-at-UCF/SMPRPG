...
class BronzeHatchet(itemService: ItemService, type: CustomItemType) : BronzeAttributeItem(itemService, type),
    ...
    override fun getAttributeModifiers(item: ItemStack?): MutableCollection<AttributeEntry?> {
        return mutableListOf(
            AdditiveAttributeEntry(AttributeWrapper.MINING_POWER, ToolGlobals.BRONZE_TOOL_MINING_POWER.toDouble()),
            AdditiveAttributeEntry(
                AttributeWrapper.STRENGTH,
                ItemSword.getSwordDamage(CustomItemType.BRONZE_SWORD) - 5
            ),
