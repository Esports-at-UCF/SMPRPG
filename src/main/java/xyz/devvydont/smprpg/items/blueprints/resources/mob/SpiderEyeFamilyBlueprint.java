...
public class SpiderEyeFamilyBlueprint extends CustomItemBlueprint implements ICompressible, ISellable, IEdible, IConsumable {
    ...
    public CompressionStep getDecompressor() {
        return switch (getCustomItemType()) {
            case PREMIUM_SPIDER_EYE -> new CompressionStep((ICompressible) itemService.getVanillaBlueprint(ItemStack.of(Material.SPIDER_EYE)), 1, 9);
            case ENCHANTED_SPIDER_EYE -> new CompressionStep((ICompressible) itemService.getBlueprint(CustomItemType.PREMIUM_SPIDER_EYE), 1, 9);
            default -> null;
        };
    }
    ...
    public CompressionStep getCompressor() {
        return switch (getCustomItemType()) {
            case PREMIUM_SPIDER_EYE -> new CompressionStep((ICompressible) itemService.getBlueprint(CustomItemType.ENCHANTED_SPIDER_EYE), 9, 1);
            default -> null;
        };
    }
