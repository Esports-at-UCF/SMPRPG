...
class BinaryLog(itemService: ItemService, type: CustomItemType) :
    ...
    override val compressor: CompressionStep? get() = when (customItemType) {
        CustomItemType.SULFUR ->
            CompressionStep(itemService.getBlueprint(CustomItemType.PREMIUM_BINARY_LOG) as ICompressible, 9, 1)
        else -> null
    }
