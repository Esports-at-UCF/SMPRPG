...
class WarlockHood(itemService: ItemService, type: CustomItemType) : WarlockArmorSet(itemService, type) {
    override fun getAttributeModifiers(item: ItemStack): MutableCollection<AttributeEntry?> {
        ...
        attributes.add(AdditiveAttributeEntry(AttributeWrapper.INTELLIGENCE, 300.0))
    ...
    fun onValidEntityKill(event: EntityDeathEvent) {
        ...
        if (killer != null) {
            if (entity.persistentDataContainer.getOrDefault(
                ...
                if (blueprint(item) !is WarlockHood) return
    ...
    override fun getDisplayKey(): Key? {
        return NamespacedKey(plugin, "illager_warlock_hood")
    }
}
