...
abstract class RoseGoldArmorSet(itemService: ItemService, type: CustomItemType) :
    ...
    val armorDurabilityUnit: Int get() = ToolStats.ROSE_GOLD.getArmorUnitDurability().toInt()

    override fun getAssetId(): Key { return key }

    override fun getPowerRating(): Int { return ToolStats.ROSE_GOLD.power }

    open fun getCraftingMaterial(): ItemStack = itemService.getCustomItem(CustomItemType.ROSE_GOLD_INGOT)

    open fun unlockedBy(): MutableCollection<ItemStack?>? { return mutableListOf(itemService.getCustomItem(CustomItemType.ROSE_GOLD_INGOT)) }
    ...
    companion object {
        private val key = Key.key("rose_gold")
    }
}
