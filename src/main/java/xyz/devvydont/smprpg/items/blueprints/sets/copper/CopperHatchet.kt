...
class CopperHatchet(itemService: ItemService, type: CustomItemType) : CustomAttributeItem(itemService, type),
    ...
    override fun getAttributeModifiers(item: ItemStack?): MutableCollection<AttributeEntry?> {
        return mutableListOf(
            AdditiveAttributeEntry(AttributeWrapper.MINING_POWER, ToolStats.COPPER.miningPower.toDouble()),
            AdditiveAttributeEntry(AttributeWrapper.STRENGTH, getSwordDamage(Material.COPPER_SWORD) - 5),
            MultiplicativeAttributeEntry(AttributeWrapper.ATTACK_SPEED, ItemAxe.Companion.AXE_ATTACK_SPEED_DEBUFF),
            AdditiveAttributeEntry(AttributeWrapper.MINING_SPEED, ToolStats.COPPER.speed * 0.8),
            AdditiveAttributeEntry(AttributeWrapper.WOODCUTTING_FORTUNE, ToolStats.COPPER.fortune * 0.8),
            AdditiveAttributeEntry(AttributeWrapper.FARMING_FORTUNE, ToolStats.COPPER.fortune * 0.8)
        )
    }
