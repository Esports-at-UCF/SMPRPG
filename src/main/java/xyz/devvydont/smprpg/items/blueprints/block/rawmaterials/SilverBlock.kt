...
class SilverBlock(itemService: ItemService, type: CustomItemType) : BlockBlueprint(itemService, type),
    ...
    override fun getWorth(item: ItemStack): Int {
        val ingot = itemService.getCustomItem(CustomItemType.MITHRIL_INGOT)
    ...
    override val decompressor: CompressionStep? get() = when (customItemType) {
        CustomItemType.SILVER_BLOCK ->
            CompressionStep(itemService.getBlueprint(CustomItemType.SILVER_INGOT) as ICompressible, 1, 9)
        else -> null
    }

    override val compressor: CompressionStep? get() = when (customItemType) {
        CustomItemType.SILVER_BLOCK ->
            CompressionStep(itemService.getBlueprint(CustomItemType.ENCHANTED_SILVER) as ICompressible, 9, 1)
        else -> null
    }
}
