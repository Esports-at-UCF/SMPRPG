...
class SteelSpear(itemService: ItemService, type: CustomItemType) : SteelAttributeItem(itemService, type), ICraftable, IBreakableEquipment, IModelOverridden {
    ...
    companion object {
        val KINETIC_COMP : KineticWeapon = KineticWeapon.kineticWeapon()
            .dismountConditions(KineticWeapon.condition(50, 7.5f, 0.0f))
            .forwardMovement(0.38f)
            .hitSound(SoundEventKeys.ITEM_SPEAR_HIT)
            .sound(SoundEventKeys.ITEM_SPEAR_USE)
            .knockbackConditions(KineticWeapon.condition(130, 5.1f, 0.0f))
            .delayTicks(10)
            .damageMultiplier(0.95f)
            .damageConditions(KineticWeapon.condition(220, 0.0f, 4.6f))
            .build()
    }
}
