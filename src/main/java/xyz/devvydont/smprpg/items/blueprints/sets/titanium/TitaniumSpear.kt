...
class TitaniumSpear(itemService: ItemService, type: CustomItemType) : TitaniumAttributeItem(itemService, type), ICraftable, IBreakableEquipment, IModelOverridden {
    ...
    companion object {
        val KINETIC_COMP : KineticWeapon = KineticWeapon.kineticWeapon()
            .dismountConditions(KineticWeapon.condition(80, 12.0f, 0.0f))
            .forwardMovement(0.38f)
            .hitSound(SoundEventKeys.ITEM_SPEAR_HIT)
            .sound(SoundEventKeys.ITEM_SPEAR_USE)
            .knockbackConditions(KineticWeapon.condition(190, 5.1f, 0.0f))
            .delayTicks(11)
            .damageMultiplier(0.8f)
            .damageConditions(KineticWeapon.condition(270, 0.0f, 4.6f))
            .build()
    }
}
