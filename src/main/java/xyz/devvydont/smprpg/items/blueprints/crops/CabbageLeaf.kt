...
class CabbageLeaf(itemService: ItemService, type: CustomItemType) : CraftEngineBlueprint(itemService, type), IEdible,
    ISellable {
    override val itemClassification: ItemClassification get() = ItemClassification.CONSUMABLE

    override fun getWorth(itemStack: ItemStack): Int { return 2 * itemStack.amount }

    override fun getNutrition(item: ItemStack?): Int { return 2 }
    ...
    override fun getConsumableComponent(item: ItemStack?): Consumable {
        return Consumable.consumable()
            .consumeSeconds(IEdible.DEFAULT_EAT_SPEED / 2.0f)
            .build()
    }
}
