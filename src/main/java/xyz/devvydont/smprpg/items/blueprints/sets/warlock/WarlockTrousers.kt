...
class WarlockTrousers(itemService: ItemService?, type: CustomItemType?) : WarlockArmorSet(itemService, type),
    ...
    override fun getAttributeModifiers(item: ItemStack): MutableCollection<AttributeEntry?> {
        ...
        attributes.add(AdditiveAttributeEntry(AttributeWrapper.DEFENSE, 270.0))
        attributes.add(AdditiveAttributeEntry(AttributeWrapper.INTELLIGENCE, 175.0))
    ...
    fun onValidEntityKill(event: EntityDeathEvent) {
        ...
        if (killer != null) {
            if (entity.persistentDataContainer.getOrDefault(
                ...
                val item = killer.inventory.getItem(EquipmentSlot.LEGS)
                if (blueprint(item) !is WarlockTrousers) return
