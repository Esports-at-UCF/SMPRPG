...
class CanadianReforge(type: ReforgeType) : ReforgeBase(type) {
    override fun getAttributeModifiersWithRarity(rarity: ItemRarity): List<AttributeEntry> {
        return listOf(
            AdditiveAttributeEntry(AttributeWrapper.WOODCUTTING_FORTUNE, getSplinteringBonus(rarity).toDouble()),
    ...
    override val description: List<Component>
        get() = listOf<Component>(
            ...
            ComponentUtils.create("Splintering and non-combat reach")
        )

    override val powerRating: Int get() = 3

    companion object {
        fun getSplinteringBonus(rarity: ItemRarity): Float {
