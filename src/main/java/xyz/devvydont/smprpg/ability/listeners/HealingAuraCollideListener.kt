...
import xyz.devvydont.smprpg.ability.handlers.HealingAuraAbilityHandler
 ...
 */
class HealingAuraCollideListener : ToggleableListener() {
    /*
    ...
    private fun onAuraProjectileHit(event: ProjectileHitEvent) {
        ...
        if (!HealingAuraAbilityHandler.Companion.isAuraProjectile(event.getEntity())) return
        ...
        if (player != null) {
            ...
            loc.yaw = 0f
            loc.pitch = 0f
            HealingAuraAbilityHandler.HealingAuraTask(loc, player)
            HealingAuraAbilityHandler.Companion.removeAuraProjectile(event.getEntity())
        }
    }
}
