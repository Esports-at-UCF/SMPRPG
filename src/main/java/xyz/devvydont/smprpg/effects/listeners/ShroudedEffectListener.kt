 ...
 */
class ShroudedEffectListener(private val service: SpecialEffectService) : Listener {
    ...
    private val randomMessage: String
        get() = messages[(Math.random() * messages.size).toInt()]
    ...
    private fun onRespawn(event: PlayerRespawnEvent) {
        ...
        val msg = ComponentUtils.merge(
            ...
            ComponentUtils.create("\"" + this.randomMessage + "\"", NamedTextColor.AQUA)
        ).hoverEvent(
            HoverEvent.showText(
