...
class MenuWithdrawWallet(owner: Player, val wallet: ItemStack, val walletBp: WalletBlueprint) : MenuBase(owner, 3) {
        ...
        this.player.sendMessage(
            ComponentUtils.success(
                ComponentUtils.merge(
                    ...
                    ComponentUtils.create(formatMoney(getWalletBalance()), NamedTextColor.GOLD)
                )
            )
        )
    }
    ...
    private fun renderMenu() {
        ...
        var currentSlot = 10
        val playerBalance = getWalletBalance()
    ...
    private fun performWithdrawal(coin: CustomItemCoin, desiredStackSize: Int) {
        ...
        val moneyTakenSuccessfully = takeCoinsFromWallet(totalCost.toInt())
        ...
        this.player.sendMessage(
            ComponentUtils.success(
                ComponentUtils.merge(
                    ...
                    ComponentUtils.create(" from your wallet! ", NamedTextColor.GREEN),
                    ComponentUtils.create("Your wallet now has ", NamedTextColor.GREEN),
                    ComponentUtils.create(formatMoney(getWalletBalance()), NamedTextColor.GOLD)
                )
            )
        )
    ...
    fun getWalletBalance(): Long {
        return walletBp.maxCoins - wallet.getData(DataComponentTypes.DAMAGE)!!.toLong()
    }

    fun takeCoinsFromWallet(amtToTake: Int): Boolean {
        val currCoins = getWalletBalance().toInt()
        if (currCoins - amtToTake < 0) return false
        else {
            wallet.setData(DataComponentTypes.DAMAGE, walletBp.maxCoins - (currCoins - amtToTake))
