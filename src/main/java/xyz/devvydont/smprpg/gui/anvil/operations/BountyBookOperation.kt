 ...
 */
object BountyBookOperation : AnvilOperation {
    ...
    override fun tryApply(player: LeveledPlayer, primary: ItemStack, secondary: ItemStack): AnvilResult? {
        ...
        if (ItemService.blueprint(secondary) !is BookOfBounties) return null
        ...
        when (blueprint.itemClassification) {
            ...
            ItemClassification.ROD -> {
                // Make sure we aren't capped out on books first
                val books = primary.persistentDataContainer.getOrDefault(
                    BookOfBounties.BOUNTY_BOOK_KEY, PersistentDataType.INTEGER, 0
                )
                if (books >= BookOfBounties.MAX_BOUNTY_BOOKS) return null

                val result = primary.clone()
                BookOfBounties.addBountyBookToItem(result)
