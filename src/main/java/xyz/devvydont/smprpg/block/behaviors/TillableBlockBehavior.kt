...
class TillableBlockBehavior(customBlock: CustomBlock,
                            val tilledBlockKey: Key): BukkitBlockBehavior(customBlock) {
    ...
    override fun useOnBlock(context: UseOnContext, state: ImmutableBlockState): InteractionResult {
        ...
        if (item.hasItemTag(Key.of("minecraft:hoes"))) {
            // Till our block
    ...
    companion object {
        ...
        class Factory : BlockBehaviorFactory<BlockBehavior> {
            override fun create(block: CustomBlock, arguments: Map<String, Any>): TillableBlockBehavior {
                val tilledBlock = Key.of(
                    ResourceConfigUtils.requireNonEmptyStringOrThrow(
                        arguments.get("tilled-block"),
                        "warning.config.block.behavior.tillable_block.missing_block"
                    )
                )
